...
for model in "${models[@]}"; do
  ...
  if [ -z "$current_periodic_time" ]; then
      echo "Error: Periodic timing not found for model $model. Skipping experiments."
      continue
  fi

  echo "Using Periodic Time: $current_periodic_time sec"
  echo "IMPORTANT: Ensure 'msTestPhasePeriod' is set manually in config.yaml before proceeding with experiments for $model."
