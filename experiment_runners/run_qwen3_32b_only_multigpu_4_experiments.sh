#!/bin/bash
# Script to run Qwen3-32B 4-GPU experiments only.
...
run_experiments() {
    ...
    local output_.venvile="${none_tuning_dir}/EP_stdout"
...
run_all_experiments() {
    for i in $(seq 1 5); do
        ...
        run_experiments "qwen3_32b_4_multigpu" "0,1,2,3" 800 20

        # Move results to a run-specific folder
        local run_dir="run_$i"
        mkdir -p "$run_dir"
        mv "qwen3_32b_4_multigpu_results" "$run_dir/"
...
# msTestPhasePeriod=800
run_all_experiments

echo "Qwen3-32B 4-GPU experiments completed."
