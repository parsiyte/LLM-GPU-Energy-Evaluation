#!/bin/bash
# Script to run Qwen3-14B 2-GPU experiments only.
...
run_experiments() {
    ...
    for metric in 0 1 2; do
        ...
        for wait_phase in 0 1; do
            ...
            ../../split/build/apps/DEPO/DEPO --gss --gpu $depo_gpu_args "$model_script" 2>&1 | tee "${exp_folder_path}/EP_stdout"
...
# --- Function to run all experiments 5 times ---
run_all_experiments() {
    for i in $(seq 1 5); do
        echo "================================================="
        ...
        run_experiments "qwen3_14b_multigpu" "0,1" 3200 80

        # Move results to a run-specific folder
        local run_dir="run_$i"
        mkdir -p "$run_dir"
        mv "qwen3_14b_multigpu_results" "$run_dir/"
        
        echo "Results for run $i saved in $run_dir"
        echo "================================================="
        echo "                 COMPLETED RUN $i of 5             "
        echo "================================================="
        echo
    done
}
...
# NOTE: msTestPhasePeriod and periodic_time may need adjustment
# msTestPhasePeriod=3200
run_all_experiments

echo "Qwen3-14B 2-GPU experiments completed."
