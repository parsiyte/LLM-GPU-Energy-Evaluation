#!/bin/bash
# Script to run Qwen3-14B 4-GPU experiments only.
...
run_all_experiments() {
    for i in $(seq 1 5); do
        ...
        run_experiments "qwen3_14b_4_multigpu" "0,1,2,3" 3200 80

        # Move results to a run-specific folder
        local run_dir="run_$i"
        mkdir -p "$run_dir"
        mv "qwen3_14b_4_multigpu_results" "$run_dir/"
...
echo "Qwen3-14B 4-GPU experiments completed."
