#!/bin/bash
# Script to run Qwen3-8B 2-GPU experiments only.
...
run_all_experiments() {
    for i in $(seq 1 5); do
        ...
        run_experiments "qwen_8b_multigpu" "0,1" 6400 120

        # Move results to a run-specific folder
        local run_dir="run_$i"
        mkdir -p "$run_dir"
        mv "qwen_8b_multigpu_results" "$run_dir/"
...
echo "Qwen3-8B 2-GPU experiments completed."
