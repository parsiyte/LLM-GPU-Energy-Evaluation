#!/bin/bash
# Script to run Qwen3-32B 2-GPU experiments only.
...
run_all_experiments() {
    for i in $(seq 1 5); do
        ...
        run_experiments "qwen3_32b_multigpu" "0,1" 1600 40

        # Move results to a run-specific folder
        local run_dir="run_$i"
        mkdir -p "$run_dir"
        mv "qwen3_32b_multigpu_results" "$run_dir/"
...
# msTestPhasePeriod=1600
run_all_experiments

echo "Qwen3-32B 2-GPU experiments completed."
