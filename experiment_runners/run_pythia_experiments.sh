...
for model in "${models[@]}"; do
  ...
  for metric in 0 1 2; do
    for wait_phase in 0 1; do # Periodic loops
      ...
      folder_name="${model#pythia_}_exp_${metrics[$metric]##--}_$label"
    ...
    folder_name="${model#pythia_}_exp_${metrics[$metric]##--}_none"
    exp_folder_path="${experiments_parent_dir}/${folder_name}"
    mkdir -p "$exp_folder_path"
...
echo "=== Phase 3: Finished Experiments ==="
echo

# === Phase 4: Consolidate Results ===
echo "=== Phase 4: Consolidating Results ==="
final_results_dir="all_pythia_run_experiments_results"
...
for model in "${models[@]}"; do
  ...
  if [ -d "$none_tuning_dir" ]; then
    echo "Moving $none_tuning_dir to $final_results_dir/"
    mv "$none_tuning_dir" "$final_results_dir/" || echo "Warning: Failed to move $none_tuning_dir for model $model"
  else
    echo "Warning: Directory $none_tuning_dir not found for model $model."
  fi

  if [ -d "$experiments_parent_dir" ]; then
    echo "Moving $experiments_parent_dir to $final_results_dir/"
    mv "$experiments_parent_dir" "$final_results_dir/" || echo "Warning: Failed to move $experiments_parent_dir for model $model"
  else
