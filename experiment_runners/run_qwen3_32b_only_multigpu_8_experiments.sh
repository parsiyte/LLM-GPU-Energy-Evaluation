#!/bin/bash
# Script to run Qwen3-32B 8-GPU experiments only.
...
run_all_experiments() {
    for i in $(seq 1 5); do
        ...
        run_experiments "qwen3_32b_8_multigpu" "0,1,2,3,4,5,6,7" 400 10

        # Move results to a run-specific folder
        local run_dir="run_$i"
        mkdir -p "$run_dir"
        mv "qwen3_32b_8_multigpu_results" "$run_dir/"
...
# msTestPhasePeriod=400
run_all_experiments

echo "Qwen3-32B 8-GPU experiments completed."
